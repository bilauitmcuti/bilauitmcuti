...
- **Workers AI binding** — required for chat. In Cloudflare Pages: Settings → Bindings → Add → **Workers AI** → variable name `AI` (production + preview). Local: `pnpm run preview` after `build:pages`. Also declared in [`wrangler.jsonc`](wrangler.jsonc). No API key secret for inference. **Default chat model** (local + production): **GLM 4.7 Flash** (`@cf/zai-org/glm-4.7-flash`). **Fallback:** `@cf/meta/llama-4-scout-17b-16e-instruct`. Overrides: `AI_CHAT_MODEL` (alias `WORKERS_AI_MODEL`), `AI_FALLBACK_MODEL`, `WORKERS_AI_USE_DEV_MODEL=1` (lower tier limits). Rollback: `AI_CHAT_MODEL=@cf/google/gemma-4-26b-a4b-it`. See `lib/ai.ts` (`AI_MODELS`, `resolveProductionChatModelChain`).
...
- `CHAT_USE_AGENT` — set to `0` or `false` to disable tool-calling agent globally (legacy full-context path). When enabled (default), **GLM** uses the agent loop with function calling on all hosts. See [`lib/chat/agent/run-agent.ts`](lib/chat/agent/run-agent.ts).
...
**Chat API (`POST /chat/api`):** Hybrid responses — cache hits return JSON `{ reply, correlationId, path }`; LLM calls with `stream: true` (default) return **SSE** (`text/event-stream`) with `token` and `done` events. Thumbs feedback posts to **`POST /chat/feedback/api`** with the assistant `correlationId`. Default model: **GLM 4.7 Flash** on all hosts (with Scout fallback).
...
- **Agent mode** (`CHAT_USE_AGENT=1`, GLM) — hybrid tool calling on Workers AI per [Cloudflare function calling](https://developers.cloudflare.com/workers-ai/features/function-calling/). Tools: `search_calendar_activities`, `get_academic_calendar`, `get_lecture_weeks`, `get_public_holidays`, `search_uitm_knowledge`, etc. Topic router narrows exposed tools. GLM uses OpenAI-style tool JSON; partner `google/*` models use `functionDeclarations` when overridden via `AI_CHAT_MODEL` / `WORKERS_AI_MODEL`.
- **Compact fallback** — injects compact API-backed context via [`lib/chat/agent/compact-fallback.ts`](lib/chat/agent/compact-fallback.ts) when the model chain has no function-calling support.
